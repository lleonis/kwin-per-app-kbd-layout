...
How to use: set up KDE to have the two desired layouts. Leave switching on 'All Windows' setting. Put the contents of this repo under `~/.local/share/kwin/scripts/per-app-kbd-layout` or under `/usr/local/share/kwin/scripts/per-app-kbd-layout` if you want to install for all users. Adjust `altLayoutApps` in `contents/code/main.js` to your needs. Restart KDE (or just kwin) and enable the new KWin script.
